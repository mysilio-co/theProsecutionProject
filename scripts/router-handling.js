...
export function removeGroupAndTagParams(router) {
  removeMultipleQueryParams([GROUP_AFFILIATION, TAG], router);
}
...
export function setSortingParams(clickedColumn, router) {
  ...
  if (clickedColumn != currentColumn) {
    ...
    newOrder =
      currentOrder == 'asc' ? 'desc' : currentOrder == 'desc' ? '' : 'asc';
  }
  newOrder == ''
    ? removeMultipleQueryParams(['sortBy', 'order'], router)
