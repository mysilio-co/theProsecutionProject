...
export default function DataVisualizerDropdowns({
  ...
  useEffect(() => {
    ...
    setResultsOptions(RESULTS_OPTIONS[0]);
  }, [categoricalOption]);

  useEffect(() => {
    setNumberOfResults(resultsOptions);
  }, [resultsOptions]);
  ...
  return (
    <div className='md:flex items-center md:mt-2 mb-5'>
        ...
        <Listbox value={dateOptions} onChange={setDateOptions}>
          {({ open }) => (
            <>
              ...
              <div className='relative mt-1 mx-4 md:mx-0'>
                <Listbox.Button className='relative w-full text-sm cursor-default rounded-md border border-gray-300 bg-white py-2 pl-1 pr-16 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-blue-500'>
                  <span className='flex items-center'>
      ...
      {chartType != CHOROPLETH && chartData?.length > 5 ? (
        <Listbox value={resultsOptions} onChange={setResultsOptions}>
          {({ open }) => (
            <>
              <Listbox.Label className='block text-sm pl-4 pr-2 font-medium text-gray-500 pt-5 md:pt-0'>
                # shown
              ...
              <div className='relative mt-1 mx-4 md:mx-0'>
                ...
                >
                  <Listbox.Options className='absolute z-10 mt-1 max-h-40 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
                    {getTopResultsDropdownArray(chartData).map((key, idx) => (
                      <Listbox.Option
                        key={idx}
                        className={({ active }) =>
                          classNames(
